...
def align_paragraph(paragraph, width, debug=0):
    ...
    if type(paragraph) == type(lines):
        ...
        raise TypeError('Unsupported paragraph type: %r' % type(paragraph))
...
if __name__ == '__main__':
    ...
    from model import ReinaValeraBible
    rv = ReinaValeraBible()('act', 1)
    v = [t[1] for t in rv[1:]]
    for l in [align_paragraph(s, width=60) for s in v]:
        for lin in l: print (lin)
